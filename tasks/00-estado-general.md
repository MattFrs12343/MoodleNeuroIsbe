...
> **Fases 1, 2 HECHAS. Fase 5 al 50% (2/4).** El desarrollador entregó 6 imágenes
> (`assets/`); se usaron para logo/favicon/fondo de hero (SDD-TEM-01/02/04). Fase 3
> sigue bloqueada en T3.3 (falta número/orden real de módulos del Dr.) → Fase 4 y
> SDD-TEM-03 (portada con tarjetas) dependen de eso. **Dos bugs reales de Moodle
> 4.5.13 encontrados y corregidos sin tocar core** (ver `specs/05-fase-tema.md`):
> (1) `$THEME->scss` como array no compilaba nada custom desde el origen del tema;
> (2) el logo/favicon vía Site admin genera URLs rotas, resuelto con fallback nativo +
> renderer override. Falta confirmación **visual** (Playwright/captura) — la extensión
> de Chrome no estaba conectada. Sigue pendiente del Dr.: acceso YouTube/Dropbox,
> conteos de contenido — ver `docs/13-info-necesaria.md` §2, §3.
...
| 5 · Tema y portada | [05-tema.md](05-tema.md) | 🟡 50% (2/4) | Logo/favicon/hero hechos; portada con tarjetas bloqueada por Fase 3/4; falta footer/cookies |
...
|---|---|---|---|
| T0.1 | Completar `docs/13-info-necesaria.md` | [x] Licencia (§4) · [x] Dominio (§1) · [x] Hosting técnico (§6) · [x] Acceso/BD (§2, falta YouTube/Dropbox) · [x] Diseño (§5, logo/imágenes recibidas) · [ ] Contenido (§3, número de módulos) | 🟡 PARCIAL (solo falta §3 y YouTube/Dropbox de §2) |
